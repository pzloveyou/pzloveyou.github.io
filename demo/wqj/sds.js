...
for (O = k = 0; x = z = j = i = k < 200;){
    var sp = M[k] = k ? c.cloneNode(0) : c;
// with(M[k] = k ? c.cloneNode(0) : c) {

    sp.width = sp.height = k ? 32 : W = 446;
    var ds = sp.getContext('2d')
// with(getContext('2d')){
    if (k > 10 | !k)
        for (
            ds.font = '60px Impact',
        ...
        for (; x = T(i),
            ...
            if (D < 1)
                ds.beginPath(
                    ds.strokeStyle = V + R + ',' + (R + B * L >> 0) + ',40,.1)'
                ),
                    ds.moveTo(U + x * 8, U + y * 8),
                    ds.lineTo(U + x * U, U + y * U),
                    ds.stroke();
// }
    for (
        y = H = k + E(k++) * 25,
